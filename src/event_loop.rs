...
impl NetworkEventLoop {
    ...
    async fn handle_peer_messages(
        ...
        debug!("Starting message handling for peer: {} with {} second timeout", peer_id, message_timeout_secs);
        ...
        loop {
            ...
            match read_result {
                ...
                Ok(Ok(n)) => {
                    ...
                    match bincode::deserialize::<NetworkMessage>(message_data) {
                        Ok(message) => {
                            ...
                            if let Err(e) = message_handler.handle_message(peer_id.clone(), message).await {
                                error!("Error handling message from peer {}: {}", peer_id, e);
                                peer_manager.mark_unhealthy(&peer_id, e.to_string()).await.ok();
                                break;
                            }
                        }
                        Err(e) => {
                            error!("Failed to deserialize message from peer {}: {}", peer_id, e);
                            peer_manager.mark_unhealthy(&peer_id, format!("Malformed message: {}", e)).await.ok();
                            break;
                        }
                    }
                    continue;
                ...
                Err(_) => {
                    ...
                    peer_manager.mark_unhealthy(&peer_id, "Message timeout".to_string()).await.ok();
                    break;
                }
            }
        }
        ...
        let _ = connection_pool.remove_connection(&peer_id).await;
        let _ = peer_manager.mark_unhealthy(&peer_id, "Connection closed".to_string()).await;
